...
const GridBuyListSidebar = ({ onApplyFilter }) => {
  ...
  const categoriesList = [
    { id: 'apartments', label: { en: 'Apartments (45)', bn: 'অ্যাপার্টমেন্ট (৪৫)' } },
  ...
  const amenitiesList = [
    ...
    { id: 'centralAir', label: { en: 'Central Air (28)', bn: 'সেন্ট্রাল এসি (২৮)' } },
    { id: 'accessible', label: { en: 'Chair Accessible (29)', bn: 'হুইলচেয়ার প্রবেশযোগ্য (২৯)' } },
  ]
                  ...
                  >
                    ...
                    <div className="flex text-amber-400 text-[11px] gap-0.5">
                      {[...Array(5)].map((_, i) => (
                        <FaStar
                          key={i}
                          className={i < stars ? 'text-amber-400' : 'text-gray-200 dark:text-slate-700'}
                        />
                      ))}
                    </div>
      ...
      <section className="col-span-12 lg:col-span-9">
        <div className="flex flex-col gap-4">
          {allData.map((property, index) => {
            ...
            const priceValue = property?.property_price_sale ? Number(property.property_price_sale) : 0;
            const displayPrice = !isNaN(priceValue) && priceValue > 0
              ? priceValue.toLocaleString(currentLang === 'bn' ? 'bn-BD' : 'en-IN')
              : null;

            // ফিচার লিস্ট অ্যারে
            const propertyFeatures = [
              { icon: <FaBed />, label: { en: 'Bedrooms', bn: 'বেডরুম' }, value: property?.property_bedrooms || 0 },
              { icon: <FaBath />, label: { en: 'Bathrooms', bn: 'বাথরুম' }, value: property?.property_bathrooms || 0 },
              { icon: <FaCar />, label: { en: 'Parking', bn: 'পার্কিং' }, value: property?.property_parking || 0 },
              { icon: <MdOutlineBalcony className="text-base" />, label: { en: 'Balcony', bn: 'বারান্দা' }, value: property?.property_balcony || 0 },
              { icon: <FaBuilding />, label: { en: 'Floor', bn: 'তলা' }, value: property?.property_floor || 0 },
              { icon: <FaFileArrowDown />, label: { en: 'Wardrobe', bn: 'ওয়ারড্রোব' }, value: property?.property_wardrobe || 0 },
              { icon: <FaTv />, label: { en: 'TV', bn: 'টিভি' }, value: property?.property_tv || 0 },
              { icon: <MdOutlineWaterDrop className="text-base" />, label: { en: 'Water Purifier', bn: 'পানির ফিল্টার' }, value: property?.property_water_purifier || 0 },
              { icon: <MdOutlineKitchen className="text-base" />, label: { en: 'Microwave', bn: 'মাইক্রোওয়েভ' }, value: property?.property_microwave || 0 },
              { icon: <FaWind />, label: { en: 'AC', bn: 'এসি' }, value: property?.property_ac || 0 },
              { icon: <FaBuilding />, label: { en: 'Fridge', bn: 'ফ্রিজ' }, value: property?.property_fridge || 0 },
              {
                icon: <FaRegCompass />,
                label: { en: 'Curtains', bn: 'পর্দা' },
                value: property?.property_available_curtains ? (currentLang === 'bn' ? 'আছে' : 'Yes') : (currentLang === 'bn' ? 'নেই' : 'No')
              },
            ];
                  ...
                  <div className="flex items-center justify-between border-t border-gray-100 dark:border-slate-800 pt-2 mt-3 text-[11px] text-gray-400 dark:text-slate-400 font-medium">
                    ...
                    <div>
                      ...
                      <span className="text-gray-600 dark:text-slate-300 font-semibold">
                        {getSafeText(property?.property_category) || 'N/A'}
                      </span>
                    </div>
                  </div>
                </div>
